...
:::tip Live Demo
For interactive examples with live rendered diagrams, check out the [live demo](/demo.html).
:::

## Grouped Workers on AWS

A classic load balancer pattern with multiple worker nodes.
...
**Result:** A load balancer distributing traffic to 5 EC2 workers that all write to an RDS database.

## Clustered Web Services

Web services organized into logical clusters.
...
**Result:** A clustered architecture with web services, database replication, and caching layer, all grouped visually.

## Event Processing on AWS

Complex event-driven architecture with nested clusters.
...
**Result:** Multi-level nested clusters showing an event processing pipeline from K8s source through workers, queue, and Lambda processors.
...
**Result:** Complex GCP architecture with IoT Core sources, Pub/Sub messaging, Dataflow processing, and multiple targets including BigQuery, Bigtable, and App Engine.
...
**Result:** A standard Kubernetes deployment pattern showing the relationship between Ingress, Service, Pods, ReplicaSet, Deployment, and HPA.
...
**Result:** A stateful application showing StatefulSet with persistent storage using PVCs and PVs.
...
**Result:** A comprehensive on-premises architecture with load balancing, microservices, HA database, caching, monitoring, and log aggregation.
...
**Result:** Same architecture with custom colored and styled edges to indicate different types of connections.

## Custom Node Example

Using external icons with custom nodes:
...
**Result:** A diagram mixing Kubernetes Pods, AWS Aurora database, and a custom node with an external RabbitMQ icon.
