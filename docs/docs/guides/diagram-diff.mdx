...
This means if you change a node's label but keep the same provider and type, it's detected as `modified` rather than `removed` + `added`.
...
Ensure the node has the same `label`, `provider`, `service`, and `type` in both versions. If any of these differ, it won't match as a modification.
