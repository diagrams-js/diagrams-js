...
Serialize the diagram to a JSON object:
...
// Returns: { name: "My Architecture", nodes: [...], edges: [...] }
```

The JSON format includes:

- `name`, `filename`, `direction`, `curvestyle`, `theme` - diagram properties
- `nodes` - array of nodes with `id`, `label`, `provider`, `type`, `resource`
- `edges` - array of connections with `from`, `to`, `label`
- `clusters` - nested groups of nodes
- `graphAttr`, `nodeAttr`, `edgeAttr` - Graphviz attributes

### Diagram.fromJSON() (Static)

Create a diagram from a JSON object or string:
