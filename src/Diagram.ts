...
export class Diagram {
  ...
  constructor(name = "", options: DiagramOptions = {}) {
    ...
    this.filename = options.filename ?? (this.name ? this.name.toLowerCase().replace(/\s+/g, "_") : "diagram");
    ...
    if (!(theme in THEMES)) {
      throw new Error(`"${theme}" is not a valid theme. Choose from: ${Object.keys(THEMES).join(", ")}`);
    }
    this.theme = theme;
   ...
   * Render the diagram
   */
  async render(): Promise<Uint8Array | string> {
    if (!this._viz) {
    ...
    const result = this._viz.render(dot, { format: format as "png" | "svg" | "pdf" | "dot" });

    if (result.status === "failure") {
      throw new Error(`Graphviz rendering failed: ${result.errors.map((e) => e.message).join(", ")}`);
    }
  ...
  async save(filepath?: string): Promise<void> {
    ...
    const path = filepath ?? `${this.filename}.${Array.isArray(this.outformat) ? this.outformat[0] : this.outformat}`;
